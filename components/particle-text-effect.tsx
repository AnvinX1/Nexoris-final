...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
  ...
  const [isDark, setIsDark] = useState(false) // Will be set properly after mount
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    const yPosition = isMobile ? canvas.height / 2.5 : canvas.height / 3
    ...
    // Generate new color based on theme
    const newColor = {
      r: isDark ? 255 : 0, // White particles in dark mode, black in light mode
      g: isDark ? 255 : 0,
      b: isDark ? 255 : 0,
    }
  ...
  const animate = () => {
    ...
    // Background with motion blur - theme responsive
    const bgColor = isDark ? "rgba(0, 0, 0, 0.1)" : "rgba(255, 255, 255, 0.1)"
  ...
  useEffect(() => {
    setMounted(true)
    // Set initial theme state based on resolvedTheme
    if (resolvedTheme) {
  ...
  }, [mounted, resolvedTheme, isDark, words])
  ...
  return (
    <div className="w-full h-full absolute inset-0">
      <canvas 
        ...
        style={{ background: canvasBackground, zIndex: 10 }} 
      />
    </div>
  )
}
